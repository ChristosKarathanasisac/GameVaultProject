...
- **Account lifecycle**: `RegistrationStatus` currently has a single member (`Completed`) — no email verification or suspend/reactivate states. This is a deliberate MVP scope decision, not an oversight; the enum is left in place to extend once those states are needed.
